  ...
  function UpdateModal() {
    ...
    checkInputs();
        ...
        const checkInputs = () => {
          ...
          const categoryValue = modalSelectCategory.value.trim();
          const fileValue = fileInput.files[0];

          // Vérifier si le titre et la catégorie ne sont pas vides
          const isValid =
            titleValue !== "" && categoryValue !== "" && fileValue;

          // Mettre à jour la couleur du bouton en fonction de la validité
          validateButton.style.backgroundColor = isValid
            ? "#1D6154"
            : "#A7A7A7";
        };

        // Ajout d'écouteurs d'événements pour vérifier les entrées lorsqu'elles changent
        titleInput.addEventListener("input", checkInputs);
        modalSelectCategory.addEventListener("change", checkInputs);
